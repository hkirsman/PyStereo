...
| `fullres_warp` | Per-Eye Inpaint (Full-Res) | depth map | Same warp strategy as Per-Eye Inpaint but at full source resolution. Downscales only for inpainting, then composites fills back via a feathered occlusion mask. Preserves maximum sharpness for the ~97% of pixels that are directly warped. |
